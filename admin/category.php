...
if($_SESSION['uname']=='root'){
    ...
    echo "<div class='bs-example4' data-example-id='contextual-table' style='overflow-x:auto;'> <table class='table table-striped table-hover  info table-bordered table-responsive'>
        <input type='button' name='addCat'  data-toggle='modal' data-target='#exampleModal'  value='Add Category' class='btn btn-success' style='float:right;margin:15px;'/>
        <tr>
            <th>Index</th>
    ...
    $name=$res['category'];
      ...
      <td>" ?> 
        ...
        <input type='button' name = 'edit' data-toggle='modal' onclick="editModelsetData(<?php echo $res['index_'] ?>,'<?php echo $res['category'] ?>')" data-target='#EditModal' class="btn btn-warning" value="Edit" id='<?php echo $res['index_'] ?>'>
...
<script>
    ...
    function editModelsetData(data,CAtname){
        $("#myID").val(data);
...
$.ajax({
   ...
   function deleteData(data){
       var delData = confirm("Are you sure want to delete "+data + " Category ?");
       if(delData == true){   
           var cat = data;
